...
class TextExtractorVisitor extends BaseVisitor {
	...
	markSource(start, end) {
		...
		if (startLine === endLine) {
			...
			this.sourceLines[startLine - 1] =
				line.slice(0, startColumn) + markerOfWidth(endColumn - startColumn) + line.slice(endColumn);
		...
		for (let i = startLine; i <= endLine; i++) {
			...
			this.sourceLines[i - 1] =
				line.slice(0, startingIndex) +
